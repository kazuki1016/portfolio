...
//index.phpのtopページの新着の店名と画像と表示
function get_new_shops($db){
  $sql = "
    SELECT
      shop_id,
      shop_name,
...
//index.phpのtopページのトピックの店名と画像と表示
function get_topic_shop_data($db){
  $sql = "
    SELECT
      shops.shop_id,
      shops.shop_name,
      shops.filename,
      COUNT(comments.shop_id) AS comment_number
    FROM
      shops
    INNER JOIN 
      comments ON shops.shop_id = comments.shop_id
    GROUP BY shops.shop_id
    ORDER BY COUNT(comments.shop_id) DESC
    LIMIT 3
  ";
  return fetch_all_query($db, $sql);
}
